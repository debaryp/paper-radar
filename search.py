...
def run_search():
    ...
    results = []
    # seen_dois enthält von Anfang an ALLE bekannten DOIs (history + zotero)
    # damit keine Wiederholungen über Blocks oder Updates hinweg möglich sind
    ...
    for block_id, block in SEARCH_BLOCKS.items():
        ...
        if all_pmids:
            unique_pmids = list(dict.fromkeys(all_pmids))  # Reihenfolge erhalten, Duplikate raus
            print(f"  Hole Metadaten für {len(unique_pmids)} PMIDs…")
            summary = pubmed_summary(unique_pmids)
            for pmid in unique_pmids:
                ...
                # Frühzeitig filtern — DOI bereits bekannt?
                if doi and doi in seen_dois:
                    print(f"  → Skip (bekannt): {paper['title'][:50]}")
                    continue
        ...
        print(f"  {len(block_papers)} neue Kandidaten (nach Vorab-Filter)")

        if not block_papers:
            print(f"  → Keine neuen Paper in diesem Block\n")
            results.append({
                "block": block_id, "block_label": block["label"],
                "title": "Keine neuen verifizierten Treffer",
                "authors": "", "year": "", "journal": "",
                "doi": "", "source": "", "no_result": True
            })
            continue

        # Sortieren nach Relevanz — beste zuerst
        block_papers.sort(key=score_relevance, reverse=True)

        # Verifikation — stoppt sobald MAX_PER_BLOCK erreicht
        verified = []
        for paper in block_papers:
            ...
            # Doppelt sicher: nochmal prüfen ob DOI schon gesehen
            if doi in seen_dois:
                continue
            ...
            if verify_doi(doi):
                ...
                seen_dois.add(doi)  # sofort in globalen Set eintragen
                print(f"    ✓ Hinzugefügt ({len(verified)}/{MAX_PER_BLOCK})")
            time.sleep(0.2)

        print(f"  → {len(verified)} verifizierte Paper\n")
